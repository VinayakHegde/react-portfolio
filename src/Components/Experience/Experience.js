...
class Experience extends Component {
    ...
    return (
      <article className="profile-content">
        <div className="content-wrapper">
          <Timeline lineColor={"#ddd"}>{this.getStories()}</Timeline>
        </div>
      </article>
    );
  }
  ...
  addAchievements(exp) {
    ...
    if (exp.achievements && exp.achievements.length) {
      let achDetails = exp.achievements.map((achievement, index) => {
        return <li key={index.toString()}>{achievement.details}</li>;
      });
      achievements = (
        <div>
  ...
  getStories() {
    if (!UserExperiences.length)
      return <span className="no-experiences">No experience found!</span>;
    let stories = UserExperiences.map((exp, ind) => {
      let dto = this.dto(exp);
      let when = dto.startDate.concat(
        " - ",
