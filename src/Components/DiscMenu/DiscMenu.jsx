...
const DiscMenu = ({onItemClick}) => {
  ...
  const styleMotion = { scale: spring(active 
    ? 1 
    : motionMatric.scale, 
    springOption
  )};
  ...
  const nextStaggeredMotion = prevMotions => prevMotions.map((_, i) => (
    ...
    ? {
        bottom: spring(
          active ? 0 : staggeredMetric.bottom,
          springOption
        ),
        opacity: spring(
          active ? 1 : staggeredMetric.opacity,
          springOption
        )
      }
    : {
  ...
  return (<div className="disc-menu-container">
    <StaggeredMotion
        ...
        <DiscGroup active={active ? "active" : "inactive"}>
          {interpolatingStyles.map((style, i) => (
            <Disc
              ...
              style={{...{style, pointerEvents: active ? "auto" : "none"}}}>
