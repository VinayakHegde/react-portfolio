...
class SkillCard extends Component {
  render() {
    ...
    return <div className="skill-card">{this.getSkill(bar)}</div>;
  }
      ...
      const charts = UserSkills.map((skill, index) => {
        ...
        return (
          <div key={index + 1} className={className}>
            ...
            <ul className="chart--horiz">{this.getSkillTopics(skill)}</ul>
          </div>
        );
      });
  ...
  getScoreLevels() {
    if (!UserSkills.length)
      return <span className="no-skills">Skills not found!</span>;
    return (
      <ul className="lines">
